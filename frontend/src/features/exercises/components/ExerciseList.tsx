...
export default function ExerciseList() {
  ...
  useEffect(() => {
    const loadExercises = async () => {
      try {
        const baseUrl: string = "https://localhost:7087/api/exercises";
    ...
    <section className="space-y-4">
      ...
      <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {exercises.map((exercise) => (
          <li
            key={exercise.id}
            className="rounded-xl border border-gray-200 bg-blend-darken p-4 shadow-sm transition hover:shadow-md"
          >
            <h3>{exercise.name}</h3>
